...
export const Button = forwardRef<HTMLButtonElement | HTMLAnchorElement, Props<AS>>((props, ref) => {
  ...
  if (back) {
    ...
    return (
      <Link
        ...
        className={`${styles.commonDesign} ${disabled && styles.linkDisabled} group relative inline-flex gap-2 h-12 items-center justify-center px-6 font-medium`}
        {...linkAttributes}
      >
        <div className="ml-1 transition translate-x-1 group-hover:translate-x-0">
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
        ...
        <span>{linkAttributes.children}</span>
      </Link>
    );
  }
  ...
  if ("href" in props) {
    return (
      <Link
        ref={ref as Ref<HTMLAnchorElement>}
        className={`${styles.commonDesign} ${disabled && styles.linkDisabled} group relative inline-flex h-12 items-center justify-center px-6 font-medium`}
        {...linkAttributes}
      >
        <span>{linkAttributes.children}</span>
